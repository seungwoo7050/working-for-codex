...
1. **GitHub Actions 워크플로우**
   - Java 21 환경 구성 (Virtual Threads 지원)
   - Gradle 빌드 및 테스트 실행
...
java {
    // Java 21 for Virtual Threads support (v2.1.0에서 활용)
...
**의존성 구성:**
- v0.1.0 시점: web, data-jpa, validation, h2 기본 의존성
- 이후 마일스톤에서 점진적 추가:
  - v1.0.0: security, jwt
  - v1.2.0: cache, redis
  ...
  build:
    ...
    steps:
      ...
      - name: Grant execute permission for gradlew
        run: chmod +x gradlew || echo "gradlew not found, using gradle command"
      ...
      - name: Generate test report
        ...
        run: |
          if [ -f "./gradlew" ]; then
            ...
            gradle test --info || true
          fi
```

> **참고**: CI에서는 JDK 17을 사용하지만, 로컬 개발과 Virtual Threads(v2.1.0)를 위해서는 JDK 21이 권장됩니다. 
> 소스 코드는 Java 21 기능을 사용할 수 있도록 `sourceCompatibility = '21'`로 설정되어 있습니다.
...
- **JDK**: 17 (Temurin 배포판, CI 환경)
- **Gradle**: setup-gradle 액션으로 캐싱 자동 처리

> **로컬 개발**: Java 21을 권장 (Virtual Threads, 최신 언어 기능)

#### 4.2.3 빌드 단계
1. **코드 체크아웃**: 최신 코드 가져오기
2. **JDK 설정**: Java 21 환경 구성
...
1. **데이터베이스**
   ...
   - → v1.5.0 이후 필요 시 프로파일 전환으로 대응

2. **보안**
   - 인증/인가 없음
   - 모든 엔드포인트 공개
   - → v1.0.0에서 JWT 기반 인증 추가 예정

3. **에러 처리**
   - 기본 Spring Boot 에러 응답 사용
   - 커스텀 에러 응답 포맷 없음
   - → v1.0.0에서 GlobalExceptionHandler 추가 예정

4. **로깅/모니터링**
   - 기본 로깅만 사용
   - 메트릭/모니터링 없음
   - → v1.5.0 이후 Actuator 추가 고려
