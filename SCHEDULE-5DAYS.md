...
## 🔥 Day 3 - WebAudio & Final

> **테마**: WebAudio 완성 + native-video-editor 마무리 → **v3.2.4 완료**
> **목표 시간**: 10시간
...
| 15:00-16:00 | CI/CD 파이프라인 검증 | GitHub Actions 통과 |
| 16:00-17:00 | README 업데이트 | 최신 문서 |

### 저녁 세션 (3시간) - Spring Boot 시작

| 시간 | 버전 | 작업 | 산출물 |
|------|------|------|--------|
| 19:00-20:00 | v0.1.1~2 | Spring Boot 프로젝트 생성, 설정 | `Application.java` |
| 20:00-20:30 | v0.1.3 | HealthController 구현 | `/api/health` 동작 |
| 20:30-21:00 | v0.1.4 | GlobalExceptionHandler | 에러 처리 |
| 21:00-21:30 | v0.1.5 | GitHub Actions CI | CI 파이프라인 |
| 21:30-22:00 | v0.1.6 | 기본 테스트 작성 → **v0.1.0 완료** | 테스트 통과 |

### Day 3 체크포인트 ✅
- [ ] native-video-editor: v3.2.0 완료 (Audio Core)
- [ ] native-video-editor: v3.2.4 완료 🎉 **(프로젝트 완성!)**
- [ ] spring-patterns: v0.1.0 완료 (Bootstrap)

---

## 🔥 Day 4 - Spring Boot Foundation

> **테마**: Spring Boot 기초 ~ v1.6.0 (API Gateway)
> **목표 시간**: 12시간

### 오전 세션 (5시간) - Layered CRUD & Team RBAC

| 시간 | 버전 | 작업 | 산출물 |
|------|------|------|--------|
| 09:00-10:00 | v1.0.1~2 | JPA Entity, Repository | `Issue.java` |
| 10:00-11:00 | v1.0.3~4 | Service + Controller Layer | `IssueService.java` |
| 11:00-11:30 | v1.0.5 | DTO & Mapper | `IssueDto.java` |
| 11:30-12:00 | v1.0.6 | 페이징 & 정렬 | Pageable 지원 |
| 12:00-13:00 | v1.0.7 | 트랜잭션 롤백 테스트 → **v1.0.0 완료** | 테스트 통과 |
| 13:00-13:30 | v1.1.1 | Team, TeamMember 엔티티 | `Team.java` |

### 오후 세션 (4시간) - Batch, Cache & Search

| 시간 | 버전 | 작업 | 산출물 |
|------|------|------|--------|
| 14:00-14:30 | v1.1.2 | TeamService 구현 | `TeamService.java` |
| 14:30-15:00 | v1.1.3~4 | RBAC + 예외 처리 | 권한 검증 |
| 15:00-15:30 | v1.1.5 | 통합 테스트 → **v1.1.0 완료** | 테스트 통과 |
| 15:30-16:00 | v1.2.1~2 | DailyIssueStats + @Scheduled | 배치 작업 |
| 16:00-16:30 | v1.2.3~5 | @Cacheable + 테스트 → **v1.2.0 완료** | 캐시 동작 |
| 16:30-17:00 | v1.3.1~2 | Spring Data ES, ProductDocument | ES 설정 |

### 저녁 세션 (3시간) - Elasticsearch & Events

| 시간 | 버전 | 작업 | 산출물 |
|------|------|------|--------|
| 19:00-19:30 | v1.3.3~4 | ProductSearchService + API | 검색 서비스 |
| 19:30-20:00 | v1.3.5 | ES 동기화 → **v1.3.0 완료** | 검색 동작 |
| 20:00-20:30 | v1.4.1~2 | Order 엔티티 + ApplicationEvent | 이벤트 정의 |
| 20:30-21:00 | v1.4.3 | @EventListener | 이벤트 핸들러 |
| 21:00-21:30 | v1.4.4~5 | @Async + 테스트 → **v1.4.0 완료** | 비동기 동작 |
| 21:30-22:00 | v1.5.1 | PostgreSQL 마이그레이션 | `application-prod.yml` |

### Day 4 체크포인트 ✅
- [ ] spring-patterns: v1.0.0 완료 (Layered CRUD)
- [ ] spring-patterns: v1.1.0 완료 (Team & RBAC)
- [ ] spring-patterns: v1.2.0 완료 (Batch & Cache)
- [ ] spring-patterns: v1.3.0 완료 (Elasticsearch)
- [ ] spring-patterns: v1.4.0 완료 (Async Events)

---

## 🔥 Day 5 - Spring Boot Production Ready

> **테마**: Docker, Gateway + WebFlux, Virtual Threads, Rate Limiting, Caching → **v2.3.0 완료**
> **목표 시간**: 13시간

### 오전 세션 (5시간) - Docker & Gateway

| 시간 | 버전 | 작업 | 산출물 |
|------|------|------|--------|
| 09:00-09:30 | v1.5.2 | Dockerfile (멀티스테이지) | `Dockerfile` |
| 09:30-10:00 | v1.5.3 | docker-compose.yml | 오케스트레이션 |
| 10:00-10:30 | v1.5.4 | Redis 캐시 연동 | Redis 동작 |
| 10:30-11:00 | v1.5.5 | 전체 스택 테스트 → **v1.5.0 완료** | Docker 실행 |
| 11:00-11:30 | v1.6.1 | Gateway 모듈 생성 | `gateway/build.gradle` |
| 11:30-12:00 | v1.6.2 | Gateway 기본 설정 | `GatewayApplication.java` |
| 12:00-12:30 | v1.6.3 | 라우팅 규칙 정의 | 라우팅 동작 |
| 12:30-13:00 | v1.6.4 | 공통 필터 | `LoggingFilter.java` |
| 13:00-13:30 | v1.6.5 | Docker 통합 → **v1.6.0 완료 🎉** | Gateway 완성 |

### 오후 세션 (5시간) - WebFlux & Virtual Threads

| 시간 | 버전 | 작업 | 산출물 |
|------|------|------|--------|
| 14:00-14:30 | v2.0.1~2 | WebFlux + R2DBC 의존성 | `build.gradle` |
| 14:30-15:00 | v2.0.3~4 | 리액티브 컨트롤러/서비스 | `ProductController.java` |
| 15:00-15:30 | v2.0.5 | SSE 스트리밍 → **v2.0.0 완료** | 리액티브 전환 |
| 15:30-16:00 | v2.1.0.1~2 | Virtual Threads 설정 | `VirtualThreadConfig.java` |
| 16:00-16:30 | v2.1.1.1~2 | 블로킹 코드 래핑 | `HybridProductService.java` |
| 16:30-17:00 | v2.1.2.1~3 | 외부 API 병렬 호출 → **v2.1.0 완료** | Virtual Threads |
| 17:00-17:30 | v2.2.0.1~2 | Redis Rate Limiter | `RateLimitingFilter.java` |
| 17:30-18:00 | v2.2.0.3~4 | Rate Limit 헤더 → **v2.2.0 완료** | Rate Limiting |

### 저녁 세션 (3시간) - Caching & Final

| 시간 | 버전 | 작업 | 산출물 |
|------|------|------|--------|
| 19:00-19:30 | v2.3.0.1~2 | Redis Cache 설정 | `CachingConfig.java` |
| 19:30-20:00 | v2.3.0.3~4 | Gzip 압축 필터 → **v2.3.0 완료 🎉** | Caching & Compression |
| 20:00-21:00 | 정리 | 고성능 테스트 & 검증 | spring-patterns 완료! |
| 21:00-22:00 | 전체 프로젝트 테스트 | 모든 테스트 통과 확인 | 최종 검증 |

### Day 5 체크포인트 ✅
- [ ] spring-patterns: v1.5.0 완료 (Docker)
- [ ] spring-patterns: v1.6.0 완료 (API Gateway)
...
```
Day 3 오전 → native-video-editor v3.2.4 완료 🎉
Day 5 저녁 → spring-patterns v2.3.0 완료 🎉
```
...
### 시간 부족 시 우선순위
1. **필수**: native-video-editor v3.2.4 (네이티브 우선)
2. **필수**: spring-patterns v2.3.0 (백엔드 고성능)
3. **선택**: spring-patterns v1.6.0까지만 (기본 백엔드)
...
| 프로젝트 | 패치 버전 수 | 주요 태그 |
|----------|-------------|----------|
| spring-patterns | 45+ | v0.1.0 ~ v2.3.0 |
| native-video-editor | 60+ | v0.1.0 ~ v3.2.4 |

**핵심 프로젝트 총 커밋 포인트**: ~105개
