...
Client::Client(int clientFd)
    : fd(clientFd), hostname("localhost"), authenticated(false), registered(false) {
}
...
void Client::joinChannel(const std::string& channel) { channels.insert(channel); }
void Client::leaveChannel(const std::string& channel) { channels.erase(channel); }
bool Client::isInChannel(const std::string& channel) const {
    return channels.find(channel) != channels.end();
}

void Client::appendToRecvBuffer(const std::string& data) { recvBuffer += data; }
...
void Client::appendToSendBuffer(const std::string& data) { sendBuffer += data; }
const std::string& Client::getSendBuffer() const { return sendBuffer; }

void Client::clearSendBuffer(size_t bytes) {
    if (bytes >= sendBuffer.length()) sendBuffer.clear();
    else sendBuffer.erase(0, bytes);
}

std::string Client::getPrefix() const {
    std::ostringstream oss;
    oss << nickname;
    if (!username.empty()) oss << "!" << username;
    if (!hostname.empty()) oss << "@" << hostname;
    return oss.str();
}
