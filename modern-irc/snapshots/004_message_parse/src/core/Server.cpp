...
void Server::handleClientData(int clientFd) {
    ...
    // Process complete messages with Message parser
...
void Server::processMessage(Client& client, const Message& message) {
    std::string cmd = StringUtils::toUpper(message.getCommand());

    // For now, just echo the parsed message back
    std::cout << "Received from fd=" << client.getFd() << ": "
              << "CMD=" << cmd;

    if (message.getParamCount() > 0) {
        std::cout << " PARAMS=[";
        for (size_t i = 0; i < message.getParamCount(); ++i) {
            if (i > 0) std::cout << ", ";
            std::cout << message.getParam(i);
        }
        std::cout << "]";
    }
    ...
    // Echo back for now (commands will be implemented in snapshot 005)
    sendToClient(client, message.toString());
}
