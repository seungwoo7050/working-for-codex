...
> **참고**: ExternalApiService는 테스트 가능성을 위해 `externalApiUrl`을 생성자 주입받도록 설계되었습니다.
> - `@Value("${external.api.url:...}")`로 URL을 설정 가능
> - MockWebServer와 함께 사용하여 격리된 단위 테스트 가능
> - REQUEST_TIMEOUT(3초)과 RETRY_DELAY(200ms)가 상수로 정의됨
...
class ExternalApiServiceTest {
    ...
    @Test
    void fetchExternalData_재시도후_성공() {
