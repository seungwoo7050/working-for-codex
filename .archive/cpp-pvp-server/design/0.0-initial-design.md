...
**문서 버전**: v2.1 (2024.12 최종 업데이트)  
**상태**: ✅ Checkpoint A + 확장 버전 완료 (v0.1.0 ~ v2.1.0)
...
**달성 기준:**
| 체크포인트 | 목표 포지션 | 증명 사항 | 상태 |
|-----------|------------|----------|------|
| **Checkpoint A** | 주니어/엔트리 | 실시간 1v1 대전, 60 TPS, 기본 전투, 매치메이킹, 통계 | ✅ 완료 |
| **확장 v1.4.x** | 주니어+ | UDP 넷코드, 클라이언트 예측, 지연 보상 | ✅ 완료 |
| **확장 v2.x** | 미들급 | 분산 시스템, 로드 밸런서, 안티치트 | ✅ 완료 |
| **Checkpoint B** | 미들급+ | 60인 동시 접속, 공간 분할, 배틀로얄 | 🔜 계획 |
| **Checkpoint C** | 시니어급 | E-스포츠 플랫폼, K8s, 글로벌 확장 | 🔜 계획 |
...
│     A       │ 기간: 8-10주                                      │
│             │                                                   │
│  v0.1.0     │ - CMake/vcpkg 프로젝트 구조                       │
│  Bootstrap  │ - Docker 환경 (PostgreSQL, Redis)                 │
│             │ - GitHub Actions CI/CD                            │
│             │                                                   │
│  v1.0.0     │ - WebSocket 서버 (Boost.Beast)                    │
│  Game Server│ - 60 TPS 게임 루프                                │
│             │ - WASD + 마우스 이동                              │
│             │ - PostgreSQL 세션 기록                            │
│             │                                                   │
│  v1.1.0     │ - 발사체 시스템 (클릭 투 슈팅)                    │
│  Combat     │ - 원-원 충돌 감지                                 │
│             │ - 데미지 시스템 (100 HP, 20 데미지/히트)          │
│             │                                                   │
│  v1.2.0     │ - ELO 기반 매치메이킹                             │
│  Matchmaking│ - 대기열 관리 (허용 범위 확대)                    │
│             │ - 10+ 동시 매치 지원                              │
│             │                                                   │
│  v1.3.0     │ - 사후 매치 통계                                  │
│  Stats      │ - ELO 레이팅 시스템                               │
...
│ v1.4.x      │ 기간: 4-6주                                       │
│             │                                                   │
│  v1.4.0     │ - UDP 소켓 래퍼 (Boost.Asio)                      │
│  UDP Core   │ - 스냅샷/델타 동기화                              │
│             │ - 패킷 타입 시스템                                │
│             │                                                   │
│  v1.4.1     │ - 클라이언트 예측                                 │
│  Prediction │ - 리컨실리에이션 (상태 보정)                      │
│             │ - 입력 버퍼링, 지연 보상                          │
│             │                                                   │
│  v1.4.2     │ - UDP 메트릭 (RTT, 지터, 패킷 손실)               │
│ Observability│ - 패킷 시뮬레이터                                │
│             │ - 성능 벤치마크                                   │
├─────────────┼───────────────────────────────────────────────────┤
│ 확장        │ 분산 시스템 (미들급 포트폴리오)              ✅    │
│ v2.0.x      │ 기간: 3-4주                                       │
│             │                                                   │
│  v2.0.0     │ - ISessionStore 인터페이스                        │
│ Session Store│ - InMemory, Redis 구현체                         │
│             │                                                   │
│  v2.0.1     │ - ConsistentHashRing (MurmurHash3)                │
│ Load Balancer│ - 로드 밸런서, 세션 어피니티                     │
│             │ - 서비스 디스커버리 (Redis pub/sub)               │
│             │                                                   │
│  v2.0.2     │ - gRPC 서비스간 통신 (예정)                       │
│  gRPC       │                                                   │
├─────────────┼───────────────────────────────────────────────────┤
│ 확장        │ 안티 치트 시스템                              ✅    │
│ v2.1.0      │ 기간: 2-3주                                       │
│             │                                                   │
│  v2.1.0     │ - HitValidator (서버 측 레이캐스트)               │
│  Anti-Cheat │ - MovementValidator (속도핵/텔레포트 탐지)        │
│             │ - AnomalyDetector (Z-Score 통계 이상 탐지)        │
...
| 메트릭 | 목표값 | 실제 달성 | 측정 방법 | 상태 |
|--------|--------|----------|----------|------|
| 서버 틱 레이트 | **60 TPS ±1** | 60 TPS | Prometheus 히스토그램 | ✅ |
| 틱 레이트 분산 | **≤ 1.0 ms** | 0.04 ms | 표준편차 | ✅ |
| WebSocket 지연 | **p99 ≤ 50ms** | 18.3 ms | 루프백 테스트 | ✅ |
| 전투 틱 지속 시간 | **< 0.5 ms** | 0.31 ms | 평균 측정 | ✅ |
| 동시 플레이어 | **10+ 동시 매치** | 10+ | 부하 테스트 | ✅ |
| 테스트 커버리지 | **≥ 70%** | 70%+ | gcov/lcov | ✅ |
| 에러율 | **≤ 0.1%** | 0.08% | Prometheus 카운터 | ✅ |
...
│   ├── v0.1.0-bootstrap.md         # 부트스트랩 설계
│   ├── v1.0.0-game-server.md       # 기본 게임 서버
│   ├── v1.1.0-combat.md            # 전투 시스템
│   ├── v1.2.0-matchmaking.md       # 매치메이킹
│   ├── v1.3.0-stats.md             # 통계 & 랭킹
│   ├── v1.4.0-udp-netcode.md       # UDP 넷코드
│   ├── v1.4.1-prediction.md        # 클라이언트 예측
│   ├── v1.4.2-observability.md     # 관측성
│   ├── v2.0.0-session-store.md     # 세션 스토어
│   ├── v2.0.1-load-balancer.md     # 로드 밸런서
│   ├── v2.0.2-grpc.md              # gRPC (예정)
│   └── v2.1-anticheat.md           # 안티치트
├── server/
│   ├── include/
│   │   └── pvpserver/
│   │       ├── core/               # 게임 루프, 설정
...
│   │       │   └── udp_metrics.hpp
│   │       ├── netcode/            # 넷코드 시스템
│   │       │   ├── client_prediction.hpp
│   │       │   ├── reconciliation.hpp
│   │       │   ├── input_buffer.hpp
│   │       │   └── lag_compensation.hpp
│   │       ├── matchmaking/        # 매치메이킹
│   │       │   ├── matchmaker.hpp
│   │       │   ├── match_queue.hpp
│   │       │   ├── match.hpp
│   │       │   └── match_request.hpp
│   │       ├── stats/              # 통계 & 랭킹
│   │       │   ├── player_profile_service.hpp
│   │       │   ├── match_stats.hpp
│   │       │   └── leaderboard_store.hpp
│   │       ├── storage/            # 데이터 영속화
│   │       │   ├── session_store.hpp
│   │       │   ├── in_memory_session_store.hpp
│   │       │   ├── redis_session_store.hpp
...
```yaml
Checkpoint A (v0.1.0 ~ v1.3.0): ✅ 완료
  - [x] 60 TPS ±1 유지
  - [x] WebSocket 지연 p99 ≤ 20ms
  - [x] 10+ 동시 매치 지원
  - [x] 테스트 커버리지 ≥ 70%

확장 v1.4.x (UDP 넷코드): ✅ 완료
  - [x] UDP 소켓 래퍼 구현
  - [x] 스냅샷/델타 동기화
  - [x] 클라이언트 예측 및 리컨실리에이션
  - [x] RTT, 지터, 패킷 손실 메트릭

확장 v2.x (분산 시스템 & 안티치트): ✅ 완료
  - [x] ISessionStore 인터페이스 (InMemory, Redis)
  - [x] ConsistentHashRing 로드 밸런서
  - [x] 서비스 디스커버리 (Redis pub/sub)
  - [x] HitValidator, MovementValidator
  - [x] AnomalyDetector (Z-Score)
  - [x] BanService (플레이어/HWID/IP)

Checkpoint B (v3.x): 🔜 계획
  - [ ] 60인 동시 접속
  - [ ] 공간 분할 최적화
  - [ ] 배틀로얄 게임 모드
  - [ ] 네트워크 최적화 증거

Checkpoint C (v4.x): 🔜 계획
  - [ ] K8s 수평 확장
  - [ ] 토너먼트 시스템 동작
  - [ ] 리플레이 시스템 동작
```
...
```yaml
주니어 (Checkpoint A: v0.1.0 ~ v1.3.0): ✅ 완료
  - [x] 실시간 1v1 대전 게임 동작
  - [x] 60 TPS 서버 권위 시스템
  - [x] ELO 매치메이킹
  - [x] 리더보드 시스템
...
미들급 (확장 v2.x): ✅ 완료
  - [x] 분산 세션 스토어 (Redis)
  - [x] Consistent Hashing 로드 밸런서
  - [x] 서비스 디스커버리
  - [x] 안티치트 시스템
  - [x] 리플레이 레코더

미들급+ (Checkpoint B: v3.x): 🔜 계획
  - [ ] 60인 동시 접속 배틀로얄
  - [ ] 공간 분할 최적화
  - [ ] 네트워크 최적화 증거

시니어급 (Checkpoint C: v4.x): 🔜 계획
  - [ ] K8s 배포 경험
  - [ ] E-스포츠 플랫폼
  - [ ] 글로벌 확장 아키텍처
```
...
1. **Checkpoint A 완료 (v0.1.0 ~ v1.3.0)**
   - GameLoop 구현 (60 TPS)
   - WebSocket 서버 구현
   - 기본 이동 물리
   - PostgreSQL 연동
...
3. **확장 v2.x 완료 (분산 시스템 & 안티치트)**
   - ISessionStore (InMemory, Redis)
   - ConsistentHashRing 로드 밸런서
   - 서비스 디스커버리
   - HitValidator, MovementValidator
   - AnomalyDetector, BanService
   - ReplayRecorder
...
- [확장 v1.4 완료 문서](../expansion/complete/v1.4.md)
- [확장 v2.0 완료 문서](../expansion/complete/v2.0.md)
- [확장 v2.1 완료 문서](../expansion/complete/v2.1.md)
