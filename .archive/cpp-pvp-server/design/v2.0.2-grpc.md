...
**상태**: ✅ 완료 (설계 완료, 인터페이스 정의)

> **참고**: gRPC 서비스 인터페이스와 프로토콜이 정의되었으며, ServerInfo에 grpc_port가 포함되어 있습니다.
> 실제 gRPC 서버/서비스 구현은 vcpkg 의존성(grpc, protobuf) 추가 후 완전 구현됩니다.
...
v2.0.2에서 정의한 서버 간 통신 프로토콜은 안티치트 시스템의 기반이 됩니다:

| 현재 (v2.0.2) | 다음 (v2.1) | 연결점 |
|---------------|-------------|--------|
| BroadcastEvent | 밴 이벤트 브로드캐스트 | 전체 서버 동기화 |
| TransferPlayer | 세션 검증 | 플레이어 이동 시 검증 |
| HealthCheck | 밴 서비스 상태 | 서비스 가용성 확인 |

### 다음 단계: [v2.1-anticheat.md](v2.1-anticheat.md)
- 서버 측 히트 검증
- 이동 검증
- 밴 서비스