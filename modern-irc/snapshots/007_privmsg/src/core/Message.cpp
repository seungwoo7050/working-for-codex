...
bool Message::parse(const std::string& rawMessage) {
    ...
    while (pos < msg.length() && msg[pos] == ' ') pos++;
...
size_t Message::getParamCount() const { return params.size(); }

std::string Message::getParam(size_t index) const {
    return index < params.size() ? params[index] : "";
}

void Message::setPrefix(const std::string& p) { prefix = p; }
void Message::setCommand(const std::string& cmd) { command = cmd; }
void Message::addParam(const std::string& param) { params.push_back(param); }
void Message::setTrailing(const std::string& trail) { trailing = trail; }
bool Message::isValid() const { return !command.empty(); }
