...
├── include/pvpserver/
│   └── anticheat/
│       ├── hit_validator.h      # 히트 검증 + Vec3, AABB, Capsule, Ray
│       ├── movement_validator.h # 이동 검증
│       ├── anomaly_detector.h   # 이상치 탐지
│       ├── ban_service.h        # 밴 서비스
│       └── replay_recorder.h    # 리플레이 기록
└── src/
    └── anticheat/
        ├── hit_validator.cpp
...
v2.1은 프로젝트의 마지막 완료 버전입니다:

| 구현 완료 | 상세 |
|-----------|------|
| HitValidator | 서버 측 레이캐스트, AABB/캡슐 충돌, 지연 보상 |
| MovementValidator | 속도핵, 텔레포트, 월핵 탐지 |
| AnomalyDetector | 통계 기반 이상치 분석 |
| BanService | 밴 정책, 기간별 처리, 복합 확인 |
| ReplayRecorder | 프레임 기록 및 압축 |
...
v0.1.0 Bootstrap
    │
    ▼
v1.0.0 Game Server Core
    │
    ▼
v1.1.0 Combat System
    │
    ▼
v1.2.0 Matchmaking
    │
    ▼
v1.3.0 Stats & Leaderboard
    │
    ▼
v1.4.0 UDP Netcode
    │
    ▼
v1.4.1 Client Prediction
    │
    ▼
v1.4.2 Observability
    │
    ▼
v2.0.0 Session Store (Redis)
    │
    ▼
v2.0.1 Load Balancer
    │
    ▼
v2.0.2 gRPC Protocol
    │
    ▼
v2.1 Anti-Cheat System ← 현재 완료
```