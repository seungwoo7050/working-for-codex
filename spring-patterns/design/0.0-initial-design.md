...
├─────────────────┼────────────────────┤
│ PostgreSQL 15+  │ 프로덕션 (v1.5)     │
├─────────────────┼────────────────────┤
│ Redis 7+        │ 캐시 (v1.2, v2.3)   │
├─────────────────┼────────────────────┤
│ Elasticsearch   │ 검색 (v1.3)         │
├─────────────────┼────────────────────┤
│ Kafka           │ 이벤트 (v1.4)       │
└─────────────────┴────────────────────┘
```
...
│   │   │   │
│   │   │   ├── common/                  # 공통 모듈
│   │   │   │   ├── config/              # 설정 (Swagger, 보안, 캐시 등)
│   │   │   │   ├── controller/          # 헬스 체크, API 문서
│   │   │   │   ├── dto/                 # 공통 응답 DTO
│   │   │   │   ├── exception/           # 예외 처리
│   │   │   │   ├── security/            # JWT, 인증/인가 (v1.1)
│   │   │   │   └── service/             # 공통 서비스
│   │   │   │
│   │   │   ├── config/                  # 전역 설정
│   │   │   │   └── VirtualThreadConfig  # Virtual Threads (v2.1)
│   │   │   │
│   │   │   ├── issue/                   # 이슈 도메인 (v1.0)
│   │   │   │   ├── domain/              # 엔티티
│   │   │   │   ├── repository/          # JPA
...
│   │   │   │   └── controller/          # API
│   │   │   │
│   │   │   ├── reactive/                # WebFlux 모듈 (v2.0)
│   │   │   │   ├── controller/          # Reactive 컨트롤러
│   │   │   │   ├── service/             # Reactive 서비스
│   │   │   │   └── repository/          # R2DBC 리포지토리
│   │   │   │
│   │   │   ├── hybrid/                  # 하이브리드 모듈 (v2.1)
│   │   │   │   └── controller/          # Virtual Threads + WebFlux
│   │   │   │
...
│       └── java/com/example/training/
│
├── gateway/                             # API Gateway (v1.6)
│   ├── src/                             # Spring Cloud Gateway
│   └── build.gradle
│
├── design/                              # 설계 문서
│   ├── 0.0-initial-design.md           # 이 문서
│   ├── PATCH-INDEX.md                  # 버전 인덱스
│   ├── v0.1.0-bootstrap-ci.md          # 부트스트랩 & CI
│   ├── v1.0.0-layered-crud.md          # 레이어드 CRUD
│   ├── v1.1.0-team-rbac.md             # 팀 & RBAC
│   ├── v1.2.0-batch-stats-cache.md     # 배치, 통계, 캐시
│   ├── v1.3.0-elasticsearch.md         # Elasticsearch
│   ├── v1.4.0-async-events.md          # Kafka 비동기 이벤트
│   ├── v1.5.0-production-infra.md      # Docker, PostgreSQL, Redis
│   ├── v1.6.0-api-gateway.md           # API Gateway
│   ├── v2.0.0-webflux-r2dbc.md         # WebFlux & R2DBC
│   ├── v2.1.0-virtual-threads.md       # Virtual Threads
│   ├── v2.2.0-rate-limiting.md         # Rate Limiting
│   └── v2.3.0-caching-compression.md   # 캐싱 & 압축
│
├── snapshots/                           # 버전별 스냅샷
│   ├── 001_bootstrap/
│   ├── 002_layered-crud/
...
| v1.6.0 | API Gateway | 라우팅, 필터링, 로드 밸런싱 |
| v2.0.0 | Reactive 프로그래밍 | WebFlux, R2DBC |
| v2.1.0 | Virtual Threads | Java 21 경량 스레드 |
| v2.2.0 | Rate Limiting | API 보호, 트래픽 제어 |
| v2.3.0 | 캐싱 & 압축 | 성능 최적화 |
...
| 마일스톤 | 필수 테스트 | 커버리지 |
|---------|----------|---------|
| v0.1.0 | 컨텍스트 로딩, 헬스 체크 | 기준선 |
| v1.0.0 | CRUD, 트랜잭션 롤백 | 70%+ |
| v1.1.0 | 인증/인가 시나리오 | 70%+ |
| v1.2.0 | 캐시 hit/miss, 배치 | 70%+ |
| v1.3.0 | 검색 쿼리 정확도 | 70%+ |
| v1.4.0 | 이벤트 발행/구독 | 70%+ |
| v1.5.0 | 통합 테스트 (Testcontainers) | 80%+ |
| v1.6.0 | Gateway 라우팅, 필터 | 80%+ |
| v2.0.0 | Reactive 스트림 테스트 | 80%+ |
| v2.1.0 | Virtual Threads 동시성 | 80%+ |
| v2.2.0 | Rate Limiting 검증 | 80%+ |
| v2.3.0 | 캐싱/압축 성능 테스트 | 80%+ |
...
2. 각 마일스톤을 순차적으로 완료 (v0.1.0 → v1.0.0 → ... → v2.3.0)
3. v2.3.0 완료 후 **spring-commerce (Phase 2)**으로 진입
