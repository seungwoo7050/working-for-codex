...
서버 상태를 모니터링하고 비정상 서버를 탐지합니다. (service_discovery.h에 포함)

```cpp
// distributed/service_discovery.h (HealthChecker 클래스)
...
│       ├── load_balancer.h      # 로드 밸런서 + ServerInfo
│       └── service_discovery.h  # 서비스 디스커버리 + HealthChecker
└── src/
    └── distributed/
        ├── consistent_hash.cpp
        ├── load_balancer.cpp
...
v2.0.1에서 구축한 로드 밸런싱 시스템은 gRPC 서버간 통신의 기반이 됩니다:

| 현재 (v2.0.1) | 다음 (v2.0.2) | 연결점 |
|---------------|---------------|--------|
| ServerInfo.grpc_port | gRPC 서버 | 서버간 통신 포트 |
| ServiceDiscovery | gRPC 클라이언트 라우팅 | 서버 주소 조회 |
| LoadBalancer | 요청 분배 | gRPC 로드 밸런싱 |

### 다음 단계: [v2.0.2-grpc.md](v2.0.2-grpc.md)
- gRPC 프로토콜 정의
- 서버간 통신 (매치메이킹, 세션 전달)
- 헬스 체크 프로토콜