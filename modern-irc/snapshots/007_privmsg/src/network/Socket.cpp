...
Socket::Socket() : fd(-1), port(0) { memset(&address, 0, sizeof(address)); }
Socket::Socket(int existingFd) : fd(existingFd), port(0) { memset(&address, 0, sizeof(address)); }
Socket::~Socket() { if (fd >= 0) close(); }
